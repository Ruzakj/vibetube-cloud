...
function rPrompt(b){
 ...
 function categories(){return ["Semua",...Array.from(new Set(a.map(x=>String(x.cat||"Lainnya").trim()||"Lainnya"))).sort((x,y)=>x.localeCompare(y,"id"))]}
 function filtered(){
  const q=query.trim().toLocaleLowerCase("id");
  return a.map((x,i)=>({x,i})).filter(({x})=>(category==="Semua"||String(x.cat||"Lainnya")===category)&&(!q||[x.name,x.cat,x.text].some(v=>String(v||"").toLocaleLowerCase("id").includes(q))));
 }
