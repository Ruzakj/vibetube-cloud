...
function openRicTool(n,fromHistory=false){if(!fromHistory&&!(history.state?.vtView==="ricToolView"&&history.state?.ricTool===n))history.pushState({vtView:"ricToolView",ricTool:n},"");showView("ricToolView");let m={speedmap:["Speedometer + Arah","GPS speed, kompas & arah"],lifestats:["Life Stats","Statistik lokal Ric Space"],promptlab:["Prompt Lab","Template prompt personal"],nowplaying:["Now Playing Card","Kartu dari track VibeTube aktif"],digitalisasi:["Digitalisasi","Brankas KTP, KK, SIM & STNK"],photospots:["Photo Spot Book","Koleksi spot foto"]};if(!m[n]){showView("ricSpaceView");return}$("#ricToolView").dataset.tool=n;$("#ricToolBody").className="ric-tool-body tool-"+n;$("#ricToolTitle").textContent=m[n][0];$("#ricToolSubtitle").textContent=m[n][1];let o=rg("opens",{});o[n]=(o[n]||0)+1;rs("opens",o);({speedmap:rSpeed,lifestats:rLife,promptlab:rPrompt,nowplaying:rNP,digitalisasi:rDigital,photospots:rSpots}[n])($("#ricToolBody"))}
function rSpeed(b){
